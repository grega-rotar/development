...
type Props = {
  tags: string[]
  data: CollectionEntry<"homeLab">[]
}

export default function HomeLab({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [homeLab, setHomeLab] = createSignal<CollectionEntry<"homeLab">[]>([])

  createEffect(() => {
    setHomeLab(data.filter((entry) => 
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {homeLab().length} OF {data.length}
          </div>
          <ul class="flex flex-col gap-3">
            {homeLab().map((homeLab) => (
              <li>
                <ArrowCard entry={homeLab} />
              </li>
            ))}
          </ul>
        </div>
